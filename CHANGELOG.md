...
- **MCP 接入指南与配置命令**：新增 `docs/user/mcp.md`（首个用户文档），说明 MCP 服务器安装位置、客户端配置与故障排查；桌面壳新增 `get_mcp_config` 命令，设置 → Developer 的「复制 MCP 客户端配置」按钮可直接把含绝对路径的客户端配置写入剪贴板。
...
- **多 AI 群聊**：工作台新增「多 AI 对话」应用。一个群聊可包含 2–8 个现有角色，每个成员保留独立的人设快照与服务商/模型。消息经持久化队列由后台运行器异步处理：关闭界面后讨论仍会继续，应用重启后未完成的回合会继续执行；@ 提到的成员优先应答，成员之间也可用 @ 互相点名（可在设置中关闭），未被点名时可按「连续自动轮数」预算自行续聊。仅用户可发送 /stop、/pause、/resume 命令中止、暂停或恢复生成。每条回复按发送者归属持久化并在重新打开会话时还原；每个群可设置自己的用户身份（姓名/头像/描述）与自动对话策略，设置 → 群聊提供全局默认值，新建群聊自动继承。
- **Exa 网页搜索**：设置 → Search 新增 Exa。默认使用无需密钥的免费托管 API（有速率限制）；也可改用用户提供的 Exa API 密钥，密钥写入加密凭据库而非搜索配置。聊天输入框的搜索服务商列表同步支持 Exa。
...
- **MCP server 随桌面安装包发布**：`encorehub-mcp` 现在由引擎构建步骤一并产出，与引擎运行时库同目录打包进安装包的 `lib/`（依赖重定位与签名规则一致），`engine-runtime.json` 记录其大小与校验和；安装后可直接把外部 MCP 客户端指向该二进制。
...
- **模型元数据自动更新**：应用启动后自动检查已启用元数据 provider 的缓存，目录缺失或超过 24 小时未更新时在后台重新拉取；Model metadata 设置面板可关闭自动更新，并显示每个 provider 的上次更新时间。自动更新只刷新目录缓存，已配置模型的上下文窗口等字段仍只在添加或编辑模型时应用。
...
- **OpenAI Responses API**：供应商新增独立于 Chat Completions 的 Responses API 请求格式，支持文本、图片、工具调用、推理摘要、流式事件及响应生命周期接口。
...
- **强制推理模型标题生成**：标题请求在模型拒绝关闭 thinking/reasoning 时自动回退为 provider 默认推理模式；可恢复的首次 400 只记录为信息日志，不再误报为错误。
- **GitHub Release 标题**：手动构建未填写可选标题后缀时不再向元数据脚本传入空参数；正式发行标题仅显示版本和可选文本，预发行标题额外显示 `Pre-release`。
