...
- **多 AI 群聊**：工作台新增「多 AI 对话」应用。一个群聊可包含 2–8 个现有角色，每个成员保留独立的人设快照与服务商/模型；一条消息默认按顺序发言，用 @ 可只叫某位成员，也可切换为「智能发言」让成员自行决定是否补充。每条回复按发送者归属持久化，重新打开会话仍能还原作者；Engine 新增群成员与消息发送者存储，Gateway 新增独立的群聊流式端点。
