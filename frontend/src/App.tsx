...
export default function App() {
  ...
  const [status, setStatus] = useState<ServiceStatus>({ engine: false, gateway: false });
      ...
      <div className="flex h-screen items-center justify-center bg-surface">
        <div className="text-center space-y-6">
          ...
          <div className="flex items-center justify-center gap-4 text-xs">
            <span className={`flex items-center gap-1.5 ${status.engine ? "text-green-400" : "text-text-muted"}`}>
              {status.engine ? <Wifi className="h-3 w-3" /> : <WifiOff className="h-3 w-3" />}
              Engine {status.engine ? "ready" : "waiting..."}
            </span>
            <span className={`flex items-center gap-1.5 ${status.gateway ? "text-green-400" : "text-text-muted"}`}>
              {status.gateway ? <Wifi className="h-3 w-3" /> : <WifiOff className="h-3 w-3" />}
