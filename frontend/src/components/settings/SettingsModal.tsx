...
export default function SettingsModal() {
    ...
    return (
        ...
        >
            <aside
                className="flex w-52 shrink-0 flex-col overflow-y-auto border-r border-border bg-surface-alt p-3 max-[760px]:w-14 max-[760px]:px-2">
                ...
                <nav aria-label="Settings sections" className="space-y-3">
                    {tabGroups.map((group) => (
                        <fieldset key={group.label} className="m-0 min-w-0 border-0 p-0">
                            <legend
                                className="mb-1 w-full px-2 text-[10px] font-semibold text-text-muted max-[760px]:sr-only">
                                {group.label}
                            </legend>
                            {group.tabs.map((item) => (
                                <button
