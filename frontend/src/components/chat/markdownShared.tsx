...
//
// EncoreHub renders assistant output through react-markdown. This module owns
// the provider-agnostic pieces — the GFM plugin, the custom component map,
// link safety, and the code block shell — so the base renderer and the lazy
// math renderers (KaTeX / MathJax) share one implementation instead of
// duplicating it. Math renderers only vary the remark/rehype plugin arrays.
...
/** Plugin-list shape react-markdown accepts; derived to avoid a direct `unified` import. */
...
/** GFM is always enabled; math renderers append their own remark plugin. */
export const remarkBasePlugins: PluginList = [remarkGfm];
...
interface MarkdownCoreProps extends MarkdownRendererProps {
...
export function MarkdownCore({
	...
	size = "sm",
	muted = false,
	remarkPlugins = remarkBasePlugins,
	rehypePlugins,
}: MarkdownCoreProps) {
	return (
		<ReactMarkdown
