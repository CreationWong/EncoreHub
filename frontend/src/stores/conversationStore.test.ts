...
describe("draft mailbox", () => {
    ...
    it("keeps editing local until submit and then replaces the old branch", async () => {
        ...
        expect(sendMessageStream.mock.calls[0][8]).toMatchObject({
            replaceMessageId: user.id,
            parameters: DEFAULT_ADVANCED_PARAMETERS,
        });
    ...
    it("records one priced usage entry across multiple provider rounds", async () => {
                    ...
                    model_configs: [
                        {
                            ...
                            pricing: {
                                ...
                                completion: [{value: 10, unit: "perMTokens"}],
                            },
                        },
                    ],
                    enabled: true,
        ...
        sendMessageStream.mockImplementation(
            async (
                ...
                payload.usage = {input_tokens: 8, output_tokens: 10};
                cb.onDone(payload);
            },
        );
        ...
        const records = useContextManagementStore.getState().records;
        ...
        expect(records[0].cost).toBeCloseTo(0.000116, 9);
    });
