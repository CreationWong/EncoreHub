...
export const useConversationStore = create<ConversationState>((set, get) => ({
    ...
    sendMessage: async (content, options) => {
        ...
        const contextManagement = useContextManagementStore.getState();
        const previousMessages = get().convCache[convId]?.messages ?? messages;
        let contextSummary = contextManagement.compactions[convId];
        const modelConfig = useProviderStore
            .getState()
        ...
        // A stream may emit usage for several tool rounds, but the usage log represents one user request.
        const recordTurnUsage = (
            ...
            useContextManagementStore.getState().recordUsage({
                ...
                conversationTitle:
                    get().conversations.find((item) => item.id === convId)?.title ??
                    "Conversation",
        ...
        await chatApi.sendMessageStream(
            ...
            providerKey,
            {
                ...
                onTelemetry(durationMs) {
                    usageDurationMs = Math.max(usageDurationMs, durationMs);
                    set((s) => {
                        const entry = s.convCache[convId];
                        if (!entry) return {};
                        return cacheUpdate(s, convId, {
