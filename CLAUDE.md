...
- Mirror each component's public three-part version into its ecosystem metadata whenever `MAJOR`, `COMPATIBILITY`, or `FEATURE` changes: Frontend updates the root/frontend npm manifests plus Tauri package/config metadata, and Engine updates its Cargo workspace metadata. Cargo/npm/Tauri cannot represent the fourth `PATCH` tier, so patch-only release rolls must leave ecosystem versions unchanged. Refresh affected lockfiles in the same change.
- Use `VMAJOR.COMPATIBILITY.FEATURE.PATCH` for component versions. `MAJOR` is a formal release generation changed only by an explicit developer decision. Increment `COMPATIBILITY` for compatibility, security, or major compatibility changes. Increment `FEATURE` for new features only. Increment `PATCH` for routine fixes and small changes; reset every less-significant tier after either manual increment.
- Treat `PATCH` as the release counter for the current public version. The manual release workflow bumps it only for components with production changes since the last release tag, builds the bumped commit, and tags that commit; ordinary pushes never roll versions, and Build IDs identify individual builds.
